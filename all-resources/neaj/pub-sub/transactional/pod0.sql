...
USE [SALES]
INSERT INTO CUSTOMER (CustomerID, SalesAmount) VALUES (7,100),(8,200)
...
INSERT INTO employee (employeeID, SalaryAmount) VALUES (4,100),(5,200),(6,300)
...
CREATE TABLE employ([employID] [int] NOT NULL, [SalaryAmount] [decimal] NOT NULL)
GO 
INSERT INTO employ (employID, SalaryAmount) VALUES (1,100),(2,200),(3,300)
...
@publication = N'TransRepl', 
@description = N'Snapshot publication of database ''Sales'' from Publisher ''mssql-0''.',
@retention = 0, 
@allow_push = N'true',
@repl_freq = N'continuous',  --    @repl_freq = N'continuous', *** check it: for always be up to date and replicate changes as they occur, 
@status = N'active', 
@independent_agent = N'true'

exec sp_addpublication_snapshot @publication = N'TransRepl', 
@frequency_type = 1, 
...
USE [Sales]
exec sp_addarticle 
@publication = N'TransRepl', -- Use the existing publication name
...
EXEC dbo.sp_start_job N'MSSQL-0-Sales-TransRepl-1'
GO

USE msdb;
--distribute the publication to subscriber, for example
EXEC dbo.sp_start_job N'mssql-0-Sales-TransRepl-MSSQL-1-1'
GO


-- To check the created articles in a replication configuration in SQL Server
USE distribution; -- Replace with the name of your distribution database
...
JOIN msarticles a ON p.publication_id = a.publication_id
WHERE p.publication = 'TransRepl'; -- Repla
...
use [Sales]
INSERT INTO employee (employeeID, SalaryAmount) VALUES (7,100);
use [Sales]
select * from employee;
